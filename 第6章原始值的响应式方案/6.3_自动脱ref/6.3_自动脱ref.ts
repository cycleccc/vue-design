...
function toRef<T extends Record<string | symbol, any>>(obj: T, key: keyof T) {
    const wrapper = {
        get value() {
            return obj[key];
        },
...
function toRefs<T extends Record<string | symbol, any>>(obj: T) {
    const ret: Partial<{ [K in keyof T]: ReturnType<typeof toRef> }> = {}
